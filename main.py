       ...
       def updateDialogue():
              ...
              if (len(options) > 1): # If there are multiple options, show multiple options.
                     for option in options:
                            def updateCurrentIndex(updatedIndex=option.get("nextSceneIndex"), NPC = option.get("affection").get("affectedNPC") , affectionChange = option.get("affection").get("change") ):
                                   nonlocal currentIndex
              ...
              if(affectionCheck != None):
                     affectedNPC = affectionCheck.get("affectedNPC")
                     comparison = affectionCheck.get("comparison")
                     amount = affectionCheck.get("amount")
                     altSceneIndex = affectionCheck.get("altScene")
                     if (comparison == BIGGER):
                            if (affectedNPC.getAffectionLevel() > amount):
                                   currentIndex = altSceneIndex 
                                   cleanUp()
                                   chatButton = Button(chatButtonContainer, text='Continue >>', borderwidth=2, background="#d1aa73", foreground="black", font="roboto", command=continueDialogueToScene(altSceneIndex), padx=2, pady=2)
